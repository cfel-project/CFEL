...
public class PortalImportTask extends TimerTask {
	...
	public void run() {
		...
		for (int temp = 0; temp < 999; temp++) {// while (true) {
			...
			for (int i = 0; i < dataList.length(); i++) {
				try {
					importActivity(dataList.getJSONObject(i));
	...
	private void importActivity(JSONObject activity) {
		...
		try {
			...
			if (image_url instanceof String && !image_url.toString().endsWith("/no_image.gif") && activity.containsKey("member")) {
				String portal_image_url = (String) image_url;
				if (!portal_image_url.matches("^https?://.*")) {
					portal_image_url = Config.PORTAL_ROOT + portal_image_url;
				}
